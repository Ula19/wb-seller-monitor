...
async def silent_resync_all() -> None:
    """Тихо пересинхронизирует b2b-магазины: свежие бизнес-цены без уведомлений.

    Зовём после обновления куки, чтобы протухшие цены сменились на бизнес-цены
    и при этом не сыпались ложные «цена изменилась». Розницу не трогаем — она
    живёт на каталоге без куки, её обновит ближайший минутный цикл.
    """
    async with Session() as s:
        sellers = await repo.list_sellers(s)
    sellers = [sl for sl in sellers if sl.b2b]
