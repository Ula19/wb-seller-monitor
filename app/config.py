...
class Settings(BaseSettings):
    ...
    # скидка WB Кошелька для розничных магазинов (её нет в API — WB считает на фронте).
    # Цена розницы = product − этот %. 0 = не применять. Привязана к аккаунту, меняется редко.
    wb_wallet_discount_pct: float = 0.0
